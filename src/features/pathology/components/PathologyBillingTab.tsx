...
import {
    Search, Plus, IndianRupee, Wallet, TrendingDown, X, FlaskConical, Receipt, CreditCard, Loader2, ChevronDown,
} from 'lucide-react';
...
// Charges + payments interleaved chronologically (oldest first) -- shared by the main Ledger panel
// and each Recent Transactions row's inline-expanded view, so both render the same shape.
...
export const PathologyBillingTab: React.FC = () => {
    ...
    const ledgerRows = useMemo(
        () => interleaveLedger(eventsData?.charges ?? [], eventsData?.payments ?? []),
        [eventsData],
    );
    ...
    // ─── Recent Transactions: inline-expand a row instead of jumping to the panels above --
    // the encounterId + patientId are already on the row, so this skips the patient-search
    // round-trip entirely and loads that visit's ledger directly. ─────
    ...
    const [expandedAddCharge, setExpandedAddCharge] = useState(false);
    const [expandedAddPayment, setExpandedAddPayment] = useState(false);
    ...
    const handleExpandedCharged = () => {
        setExpandedAddCharge(false);
        if (expandedRow) loadExpandedEvents(expandedRow);
        loadRecent(true);
    };

    const handleExpandedPaid = () => {
        setExpandedAddPayment(false);
        if (expandedRow) loadExpandedEvents(expandedRow);
        loadRecent(true);
    };
    ...
    return (
        <div className="space-y-6">
                    ...
                    <CardContent className="space-y-3">
                        {!selectedPatient || !selectedEncounterId ? (
                            ...
                            <>
                                ...
                                <LedgerRowsTable rows={ledgerRows} />
                            </>
                        )}
                    </CardContent>
                </Card>
                                                        ...
                                                        <td colSpan={6} className="px-3 py-3">
                                                            {expandedLoading ? (
                                                                ...
                                                                <div className="space-y-2.5">
                                                                    <div className="flex items-center gap-2">
                                                                        <Button size="sm" className="gap-1.5 h-7 text-[11px]" onClick={() => setExpandedAddCharge(true)}>
                                                                            <Plus className="h-3 w-3" /> Add Test
                                                                        </Button>
                                                                        <Button size="sm" variant="outline" className="gap-1.5 h-7 text-[11px]" onClick={() => setExpandedAddPayment(true)}>
            ...
            {expandedRow && (
                <>
                    <AddChargesModal
                        open={expandedAddCharge}
                        onOpenChange={setExpandedAddCharge}
                        encounterId={expandedRow.id}
                        patientId={expandedRow.patientId}
                        appliesToFilter="LAB"
                        onCharged={handleExpandedCharged}
                    />
                    <AddPaymentDialog
                        open={expandedAddPayment}
                        onOpenChange={setExpandedAddPayment}
                        patientId={expandedRow.patientId}
                        encounterId={expandedRow.id}
                        netBalance={expandedEvents?.netBalance ?? 0}
                        onSaved={handleExpandedPaid}
                    />
                </>
            )}
        </div>
