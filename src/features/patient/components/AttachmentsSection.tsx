...
const AttachmentsSection: React.FC<AttachmentsSectionProps> = ({ attachments, onChange, patientId, patientName, open: controlledOpen, onOpenChange, trigger }) => {
          ...
          <table className="min-w-full text-xs">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              {pagedAttachments.map((att, idx) => {
                ...
                const href = meta?.url;
                return (
                  <tr key={`${att}-${start + idx}`} className="odd:bg-white even:bg-gray-50 dark:odd:bg-gray-900 dark:even:bg-gray-850 text-gray-800 dark:text-gray-100">
                    <td className="px-3 py-2 align-top">{start + idx + 1}</td>
                    <td className="px-3 py-2 align-top">
                      {href ? (
                        <a href={href} target="_blank" rel="noreferrer" className="font-semibold text-[12px] text-blue-700 dark:text-blue-400 hover:underline" title={att}>
                          {att}
                        </a>
                      ) : (
                        <div className="font-semibold text-[12px]" title={att}>{att}</div>
                      )}
                      <div className="text-[11px] text-gray-500 dark:text-gray-400">{meta?.type || '—'}</div>
                    </td>
                    ...
                    <td className="px-3 py-2 align-top text-[11px] text-gray-700 dark:text-gray-300">{meta?.uploadedBy || '—'}</td>
            ...
            <TabsContent value="view" className="pt-1 space-y-3">
              ...
              {hasAttachments && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="text-sm text-gray-700 dark:text-gray-300">Attachment {viewIndex + 1} of {attachments.length}</div>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow-sm flex items-start gap-3">
                    ...
                    <div className="w-full space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <div className="text-sm font-semibold text-gray-800 dark:text-gray-100">{currentAttachment}</div>
                        {currentPreview?.url && (
                          <Button
                            variant="outline"
                            size="sm"
                      ...
                      {!currentPreview && (
                        <div className="text-xs text-gray-500 dark:text-gray-400">Preview not available in this demo. Download/view will be wired to backend.</div>
                      )}
                    </div>
                  </div>
