...
export const UseStockPopover: React.FC<Props> = ({ item, onSuccess, patients }) => {
    ...
    const quickSubmit = async () => {
        ...
        try {
            ...
            toast({ title: `Used 1 ${item.unit} of ${item.itemName}` });
            onSuccess();
        } catch (e: any) {
            toast({ title: 'Could not record usage', description: e.response?.data?.Message ?? e.response?.data?.message ?? e.message, variant: 'destructive' });
        } finally {
    ...
    return (
        <div className="flex items-center gap-1">
            <Button
                size="sm"
                variant="outline"
                disabled={busy || item.qtyOnHand < 1}
                onClick={quickSubmit}
                className="h-8 px-2 rounded-lg border-slate-205 dark:border-zinc-800 text-xs font-bold text-slate-700 dark:text-zinc-300 active:scale-[0.98] transition-all bg-slate-50 hover:bg-slate-100 dark:bg-zinc-800/50 dark:hover:bg-zinc-800"
                title="Quick Use 1 (No Billing)"
            >
                {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <MinusCircle className="h-3.5 w-3.5" />}
            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <Button
                        size="sm"
                        variant="outline"
                        className="h-8 px-2 rounded-lg border-slate-205 dark:border-zinc-800 text-xs font-bold text-slate-700 dark:text-zinc-300 active:scale-[0.98] transition-all shrink-0"
                    >
                        More...
                    </Button>
                </PopoverTrigger>
