...
import { GripVertical, Save, Printer, Eye, EyeOff, Loader2, LogOut, RotateCcw, Plus, Trash2 } from 'lucide-react';
import { useDischargeFieldLayout } from '../hooks/useDischargeFieldLayout';
import {
    DEFAULT_DISCHARGE_FIELDS,
...
const CUSTOM_FIELD_TYPES: { value: DischargeFieldType; label: string }[] = [
...
const typeLabel = (t?: DischargeFieldType) => CUSTOM_FIELD_TYPES.find(x => x.value === t)?.label ?? 'Field';

interface RowProps {
    field: DischargeFieldConfigItem;
    update: (key: string, patch: Partial<DischargeFieldConfigItem>) => void;
    ...
    return (
        ...
        >
            ...
            <button
                ...
                className={`w-24 sm:w-28 shrink-0 flex items-center justify-center gap-1 h-9 rounded-md text-xs font-semibold border transition-colors ${
                    field.showInPad ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gray-100 text-gray-500 border-gray-200'
                }`}
                title="Show this field in the discharge workspace form"
            >
                {field.showInPad ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}
                {field.showInPad ? 'In workspace' : 'Hidden'}
            </button>
            ...
            <button
                ...
                title={field.showInPad ? 'Include this field on the printed discharge summary' : 'Enable “In workspace” first'}
            >
...
/**
 * Editor for a doctor's personalized discharge-summary field layout: rename fields, choose whether
 * each shows in the workspace form and/or on the printed discharge summary, drag to reorder, and add
 * custom fields. Global per doctor. Mirrors PrescriptionFieldLayoutEditor.tsx.
 */
export const DischargeFieldLayoutEditor: React.FC<Props> = ({ overrideDoctorId }) => {
    const { fields, isLoading, saveLayout, isSaving } = useDischargeFieldLayout(overrideDoctorId);
    const [items, setItems] = useState<DischargeFieldConfigItem[]>([]);

    // "Add custom field" form state
    const [showAdd, setShowAdd] = useState(false);
    const [newName, setNewName] = useState('');
    const [newType, setNewType] = useState<DischargeFieldType>('text');
    ...
    const update = (key: string, patch: Partial<DischargeFieldConfigItem>) =>
    ...
    const addCustomField = () => {
        ...
        const options = newType === 'select'
            ...
            : undefined;
        const item: DischargeFieldConfigItem = {
    ...
    const resetDefaults = () => setItems(DEFAULT_DISCHARGE_FIELDS.map(f => ({ ...f })));
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex-shrink-0 p-3 sm:p-4 bg-gradient-to-r from-brand-50 to-brand-50 dark:from-brand-900/20 dark:to-brand-900/20 border border-brand-200 dark:border-brand-700 rounded-xl">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                    <div className="flex items-center gap-2 sm:gap-3">
                        <LogOut className="h-4 w-4 sm:h-5 sm:w-5 text-brand-600" />
                        <div>
                            <h3 className="text-base sm:text-lg font-semibold text-brand-900 dark:text-brand-200">Discharge Summary Fields</h3>
            ...
            <div className="hidden sm:flex items-center gap-3 px-3 pt-3 text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                ...
                <span className="w-28 text-center">In workspace</span>
                ...
                {isLoading && items.length === 0 ? (
                    ...
                    <>
                        ...
                        <div className="mt-3">
                                ...
                                <div className="rounded-lg border border-violet-200 bg-violet-50/50 dark:border-violet-900/50 dark:bg-violet-950/20 p-3 space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        <div>
                                            ...
                                            <Input value={newName} onChange={e => setNewName(e.target.value)} className="mt-1 h-9 rounded-md" placeholder="e.g. Wound care instructions" />
                                        </div>
                                        <div>
                                            <Label className="text-[11px] font-semibold text-slate-600">Field type</Label>
                                            <select
                                                value={newType}
                                                onChange={e => setNewType(e.target.value as DischargeFieldType)}
                            ...
                            <p className="mt-2 text-[11px] text-slate-400">Custom fields appear in the discharge form and on the printed summary.</p>
                        </div>
                    </>
                )}
            </div>
...
export default DischargeFieldLayoutEditor;
